...
test("Simple learning", () => {
    const key = 0x123456789ABCDEFn;
    const hop = 0xf16c47a6;
    const fix = 0x239b3deb;
    const hop_dec = simple_learning(hop, key);
...
test("Normal learning", () => {
    const hop = 0xc2be08b0;
    const fix = 0x1ee2b949;
    const key = 0x123456789ABCDEFn;
    const man = normal_learning(fix, key);
    const hop_dec = simple_learning(hop, man);
...
test("Secure learning", () => {
    const hop = 0xcd831b4f;
    const fix = 0x11111111;
    const seed = 0x22222222;
    const key = 0x123456789ABCDEFn;
    const man = secure_learning(fix, seed, key);
    const hop_dec = simple_learning(hop, man);
...
test("Magic XOR Type-1 learning", () => {
    const hop = 0xbe94c1f2;
    const fix = 0x1088b380;
    const key = 0x123456789ABCDEFn;
    const man = magic_xor_type1_learning(fix & 0x0FFFFFFF, key);
    const hop_dec = simple_learning(hop, man);
...
test("FAAC SLH (SPA) learning", () => {
    const hop = 0xe6ed4ad2;
    const fix = 0xa060b2d6;
    const key = 0x123456789ABCDEFn;
    const seed = 0x2e60b2d2;
    const man = faac_learning(seed, key);
    const hop_dec = simple_learning(hop, man);
...
describe("KeeloqImpl", () => {
    test("Simple learning", () => {
        const a = new KeeloqImpl(0x123456789ABCDEFn, LearningTypes.SIMPLE_LEARNING, 0x39b3deb, 2, 10);
    ...
    test("Normal learning", () => {
        const a = new KeeloqImpl(0x123456789ABCDEFn, LearningTypes.NORMAL_LEARNING, 0x5999533, 2, 9);
    ...
    test("Secure learning", () => {
        const a = new KeeloqImpl(0x123456789ABCDEFn, LearningTypes.SECURE_LEARNING, 0x007b310, 2, 5)
        a.seed = 0x007b310;
